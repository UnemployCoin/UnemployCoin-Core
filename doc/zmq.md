...
The UnemployCoin Core daemon can be configured to act as a trusted "border
router", implementing the unemploycoin wire protocol and relay, making
...
The ZeroMQ feature in UnemployCoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building unemploycoind:
...
For instance:
    ...
    $ unemploycoind -zmqpubhashtx=tcp://127.0.0.1:28766 \
               -zmqpubrawtx=ipc:///tmp/unemploycoind.tx.raw
...
These options can also be provided in unemploycoin.conf.
...
From the perspective of unemploycoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into unemploycoind directly. Furthermore, no information is
