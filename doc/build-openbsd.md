...
This guide describes how to build unemploycoind and command-line utilities on OpenBSD.
...
This makes it necessary to build boost, or at least the parts used by UnemployCoin Core, manually:

```
# Pick some path to install boost to, here we create a directory within the unemploycoin directory
UNEMPLOYCOIN_ROOT=$(pwd)
BOOST_PREFIX="${UNEMPLOYCOIN_ROOT}/boost"
...
# Build w/ minimum configuration necessary for unemploycoin
...
```bash
# Pick some path to install BDB to, here we create a directory within the unemploycoin directory
UNEMPLOYCOIN_ROOT=$(pwd)
BDB_PREFIX="${UNEMPLOYCOIN_ROOT}/db4"
mkdir -p $BDB_PREFIX
