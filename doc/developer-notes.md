...
The UnemployCoin Doxygen documentation can be generated by running `make docs` from the top-level 
...
UnemployCoin Core is a multithreaded application, and deadlocks or other multithreading bugs
...
However, in open source software such as UnemployCoin Core, where everyone uses
...
pay attention to for reviewers of UnemployCoin Core code.

General UnemployCoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: UnemployCoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against UnemployCoin so that they can be integrated
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:unemploycoin/unemploycoin.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `unemploycoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `unemploycoin-cli` and the GUI debug console use this table to determine how to
