UnemployCoin Core version *0.15.0* is now available from:

  <https://unemploycoin.org/bin/unemploycoin-core-0.15.0/>
...
- The `estimatesmartfee` RPC interface has been changed (See [PR 10707](https://github.com/bitcoin/bitcoin/pull/10707)):
    ...
    - If UnemployCoin Core has not been running for long enough and has not seen enough blocks or transactions to produce an accurate fee estimation, an error will be returned (previously a value of -1 was used to indicate an error, which could be confused for a feerate).
- A new `estimaterawfee` RPC is added to provide raw fee data. External clients can query and use this data in their own fee estimation logic.

Multi-wallet support
--------------------

UnemployCoin Core now supports loading multiple, separate wallets (See [PR 8694](https://github.com/bitcoin/bitcoin/pull/8694), [PR 10849](https://github.com/bitcoin/bitcoin/pull/10849)). The wallets are completely separated, with individual balances, keys and received transactions.

Multi-wallet is enabled by using more than one `-wallet` argument when starting UnemployCoin, either on the command line or in the UnemployCoin config file.

**In UnemployCoin-Qt, only the first wallet will be displayed and accessible for creating and signing transactions.** GUI selectable multiple wallets will be supported in a future version. However, even in 0.15 other loaded wallets will remain synchronized to the node's current tip in the background. This can be useful if running a pruned node, since loading a wallet where the most recent sync is beyond the pruned height results in having to download and revalidate the whole blockchain. Continuing to synchronize all wallets in the background avoids this problem.

UnemployCoin Core 0.15.0 contains the following changes to the RPC interface and `unemploycoin-cli` for multi-wallet:

* When running UnemployCoin Core with a single wallet, there are **no** changes to the RPC interface or `unemploycoin-cli`. All RPC calls and `unemploycoin-cli` commands continue to work as before.
* When running UnemployCoin Core with multi-wallet, all *node-level* RPC methods continue to work as before. HTTP RPC requests should be send to the normal `<RPC IP address>:<RPC port>/` endpoint, and `unemploycoin-cli` commands should be run as before. A *node-level* RPC method is any method which does not require access to the wallet.
* When running UnemployCoin Core with multi-wallet, *wallet-level* RPC methods must specify the wallet for which they're intended in every request. HTTP RPC requests should be send to the `<RPC IP address>:<RPC port>/wallet/<wallet name>/` endpoint, for example `127.0.0.1:8766/wallet/wallet1.dat/`. `unemploycoin-cli` commands should be run with a `-rpcwallet` option, for example `unemploycoin-cli -rpcwallet=wallet1.dat getbalance`.
...
UnemployCoin Core has supported creating opt-in replace-by-fee (RBF) transactions
...
- `uptime` returns the total runtime of the `unemploycoind` server since its last start (See [PR 10400](https://github.com/bitcoin/bitcoin/pull/10400)).

Low-level RPC changes
---------------------

- When using UnemployCoin Core in multi-wallet mode, RPC requests for wallet methods must specify
...
- Error codes have been updated to be more accurate for the following error cases (See [PR 9853](https://github.com/bitcoin/bitcoin/pull/9853)):
  ...
  - `removeprunedfunds` now returns RPC_WALLET_ERROR if `unemploycoind` is unable to remove
  the transaction. Previously returned RPC_INTERNAL_ERROR.
  - `removeprunedfunds` now returns RPC_INVALID_PARAMETER if the transaction does not
  exist in the wallet. Previously returned RPC_INTERNAL_ERROR.
  - `fundrawtransaction` now returns RPC_INVALID_ADDRESS_OR_KEY if an invalid change
  address is provided. Previously returned RPC_INVALID_PARAMETER.
  - `fundrawtransaction` now returns RPC_WALLET_ERROR if `unemploycoind` is unable to create
...
- #10475 `39039b1` [RPC] getmempoolinfo mempoolminfee is a UNEMP/KB feerate (instagibbs)
...
- #9672 `46311e7` Opt-into-RBF for RPC & unemploycoin-tx (luke-jr)
...
- #10400 `1680ee0` [RPC] Add an uptime command that displays the amount of time (in seconds) unemploycoind has been running (rvelhote)
- #10683 `d81bec7` rpc: Move the `generate` RPC call to rpcwallet (laanwj)
- #10710 `30bc0f6` REST/RPC example update (Mirobit)
- #10747 `9edda0c` [rpc] fix verbose argument for getblock in unemploycoin-cli (jnewbery)
...
- #10228 `27faa6c` build: regenerate unemploycoin-config.h as necessary (theuni)
...
- #10017 `02d64bd` combine_logs.py - aggregates log files from multiple unemploycoinds during functional tests (jnewbery)
...
- #10636 `4bc853b` [qa] util: Check return code after closing unemploycoind proc (MarcoFalke)
...
- #10090 `8e4f7e7` Update unemploycoin.conf with example for pruning (coinables)
...
- #10447 `f259263` Make unemploycoind invalid argument error message specific (laanwj)
- #10495 `6a38b79` contrib: Update location of seeds.txt (laanwj)
- #10469 `b6b150b` Fixing typo in rpcdump.cpp help message (keystrike)
- #10451 `27b9931` contrib/init/unemploycoind.openrcconf: Don't disable wallet by default (luke-jr)
...
- #10651 `cef4b5c` Verify binaries from ravencore.org and unemploycoin.org (TheBlueMatt)
- #10786 `ca4c545` Add PR description to merge commit in github-merge.py (sipa)
- #10812 `c5904e8` [utils] Allow unemploycoin-cli's -rpcconnect option to be used with square brackets (jnewbery)
...
- #9999 `a328904` [LevelDB] Plug leveldb logs to unemploycoin logs (NicolasDorier)
...
- #10958 `659c096` Update to latest UnemployCoin patches for LevelDB (sipa)
