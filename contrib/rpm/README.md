...
The RPM spec file provided here is for UnemployCoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://unemploycoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[UnemployCoin git repository](https://github.com/RavenProject/UnemployCoin) but are not part of
...
Currently only `Source10` is used, for the example `unemploycoin.conf` file.
...
in the unemploycoin git repository. At present only an SVG version of the UnemployCoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the unemploycoin
...
needed when 0.12.1 is released, a proper fix is already in the UnemployCoin git
master branch.
...
idea to build UnemployCoin Core against a static version of that library compiled
...
The `unemploycoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `unemploycoin-server` package. At this point, most RPM based
...
The `unemploycoin-server` package should have SELinux support. How to properly do
...
UnemployCoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/unemploycoin/). He has been
packaging UnemployCoin for Fedora at least since 2012.
...
4. I renamed the `unemploycoin` package that contains the Qt GUI to `unemploycoin-core` as
that appears to be how the general population refers to it, in contrast to
`unemploycoin-xt` or `unemploycoin-classic`. I wanted to make sure the general population
