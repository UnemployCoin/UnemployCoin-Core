...
def main():
    ...
    # Check that the build was configured with wallet, utils, and unemploycoind
    enable_wallet = config["components"].getboolean("ENABLE_WALLET")
    enable_cli = config["components"].getboolean("ENABLE_UTILS")
    enable_unemploycoind = config["components"].getboolean("ENABLE_UNEMPLOYCOIND")
    if not (enable_wallet and enable_cli and enable_unemploycoind):
        print("No functional tests to run. Wallet, utils, and unemploycoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    # Warn if unemploycoind is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "unemploycoind"]) is not None:
            print("%sWARNING!%s There is already a unemploycoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "UNEMPLOYCOIND" not in os.environ:
        os.environ["UNEMPLOYCOIND"] = build_dir + '/src/unemploycoind' + exeext
        os.environ["RAVENCLI"] = build_dir + '/src/unemploycoin-cli' + exeext
...
class RPCCoverage:
    ...
    commands per `unemploycoin-cli help` (`rpc_interface.txt`).
