...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of UNEMPLOYCOIN NOT allowed
        BOOST_CHECK(!IsAssetNameValid("UNEMP", type));
        BOOST_CHECK(!IsAssetNameValid("UNEMPLOYCOIN", type));
        BOOST_CHECK(!IsAssetNameValid("UNEMPLOYCOIN", type));

        //- Versions of UNEMPLOYCOIN ALLOWED
        BOOST_CHECK(IsAssetNameValid("UNEMPLOYCOIN.COIN", type));
        BOOST_CHECK(IsAssetNameValid("UNEMPLOYCOIN_COIN", type));
        BOOST_CHECK(IsAssetNameValid("UNEMPSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERUNEMP", type));
        BOOST_CHECK(IsAssetNameValid("UNEMPLOYCOINSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERAVEN", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_UNEMPLOYCOINS", type));
        BOOST_CHECK(IsAssetNameValid("SEUNEMPOT", type));
    ...
    BOOST_AUTO_TEST_CASE(transfer_asset_coin_test)
    {
        ...
        CAssetTransfer asset("UNEMPLOYCOIN", 1000);
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("UNEMPLOYCOIN", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(GetParams().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
