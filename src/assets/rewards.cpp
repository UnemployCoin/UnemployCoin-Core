...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for UNEMP
    ...
    if (p_rewardSnapshot.strDistributionAsset != "UNEMP") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is UNEMP with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using UNEMP differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "UNEMP") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse UnemployCoin address (already validated during ownership snapshot creation)
