 ...
 * between unemploycoind, and unemploycoin-qt and non-server tools.
*/
...
void PrepareShutdown()
{
    ...
    RenameThread("unemploycoin-shutoff");
    ...
    FlushWallets();
#endif
    GenerateUnemployCoins(false, 0, GetParams());
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), UNEMPLOYCOIN_CONF_FILENAME));
    if (mode == HMM_UNEMPLOYCOIND)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), UNEMPLOYCOIN_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/UnemployCoin/UnemployCoin>";
    const std::string URL_WEBSITE = "<https://unemploycoin.com>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("unemploycoin-loadblk");
...
/** Sanity checks
 *  Ensure that UnemployCoin is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("UnemployCoin Core version %s\n", FormatFullVersion());
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single UnemployCoin process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", UNEMPLOYCOIN_CONF_FILENAME)).string());
    ...
    GenerateUnemployCoins(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
