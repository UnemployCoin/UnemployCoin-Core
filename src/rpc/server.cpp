...
CAmount AmountFromValue(const UniValue& value, bool p_isUNEMP)
{
    ...
    if (p_isUNEMP && !MoneyRange(amount))
...
UniValue stop(const JSONRPCRequest& jsonRequest)
{
    ...
    if (jsonRequest.fHelp || jsonRequest.params.size() > 1)
        throw std::runtime_error(
            "stop\n"
            "\nStop UnemployCoin server.");
    // Event loop will exit after current HTTP requests have been handled, so
    // this reply will get back to the client.
    StartShutdown();
    if (jsonRequest.params[0].isNum()) {
        MilliSleep(jsonRequest.params[0].getInt<int>());
    }
    return "UnemployCoin server stopping";
}
...
{
    return "> unemploycoin-cli " + methodname + " " + args + "\n";
}
