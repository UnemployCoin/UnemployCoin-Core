    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
            ...
            for (size_t j = 0; j < tx.vin.size(); j++) {
                ...
                if (GetSpentIndex(spentKey, spentInfo)) {
                    if (spentInfo.addressType == 1) {
                        delta.pushKV("address", CUnemployCoinAddress(CKeyID(spentInfo.addressHash)).ToString());
                    } else if (spentInfo.addressType == 2)  {
                        delta.pushKV("address", CUnemployCoinAddress(CScriptID(spentInfo.addressHash)).ToString());
                    } else {
                        continue;
                    }
        ...
        for (unsigned int k = 0; k < tx.vout.size(); k++) {
            ...
            if (out.scriptPubKey.IsPayToScriptHash()) {
                ...
                delta.pushKV("address", CUnemployCoinAddress(CScriptID(uint160(hashBytes))).ToString());

            } else if (out.scriptPubKey.IsPayToPublicKeyHash()) {
                std::vector<unsigned char> hashBytes(out.scriptPubKey.begin()+3, out.scriptPubKey.begin()+23);
                delta.pushKV("address", CUnemployCoinAddress(CKeyID(uint160(hashBytes))).ToString());
            } else {
                continue;
            }
...
UniValue decodeblockToJSON(const CBlock& block)
{
    ...
    result.pushKV("size", (int)::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION));
    result.pushKV("weight", (int)::GetBlockWeight(block));
    result.pushKV("height", (int)block.nHeight);
    result.pushKV("version", block.nVersion);
    result.pushKV("versionHex", strprintf("%08x", block.nVersion));
    result.pushKV("merkleroot", block.hashMerkleRoot.GetHex());
    UniValue txs(UniValue::VARR);
...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of unemploycoin addresses\n"
...
UniValue verifychain(const JSONRPCRequest& request)
{
    ...
    if (!request.params[0].isNull())
        nCheckLevel = request.params[0].getInt<int>();
    if (!request.params[1].isNull())
        nCheckDepth = request.params[1].getInt<int>();
...
UniValue getblockchaininfo(const JSONRPCRequest& request)
{
    ...
    if (IsDGWActive(chainActive.Height())) {
        ...
        obj.pushKV("DGW_activation_height",    (int) GetParams().DGWActivationBlock());
    }
    obj.pushKV("mediantime",            (int64_t)chainActive.Tip()->GetMedianTimePast());
    obj.pushKV("verificationprogress",  GuessVerificationProgress(GetParams().TxData(), chainActive.Tip()));
