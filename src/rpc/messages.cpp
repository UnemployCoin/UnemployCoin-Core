...
UniValue viewallmessages(const JSONRPCRequest& request) {
    ...
    if (!fMessaging) {
        ...
        ret.push_back("Messaging is disabled. To enable messaging, run the wallet without -disablemessaging or remove disablemessaging from your unemploycoin.conf");
        return ret;
    }
    ...
    for (auto message : setMessages) {
        ...
        obj.pushKV("Time", DateTimeStrFormat("%Y-%m-%d %H:%M:%S", message.time));
        ...
        try {
            ...
            if (message.nExpiredTime)
                obj.pushKV("Expire Time", date);
        } catch (...) {
            obj.pushKV("Expire UTC Time", message.nExpiredTime);
        }
...
UniValue subscribetochannel(const JSONRPCRequest& request) {
    ...
    if (!fMessaging) {
        throw JSONRPCError(RPC_DATABASE_ERROR, "Messaging is disabled. To enable messaging, run the wallet without -disablemessaging or remove disablemessaging from your unemploycoin.conf");
    }
...
UniValue viewmytaggedaddresses(const JSONRPCRequest& request) {
    ...
    for (auto item : myTaggedAddresses) {
        ...
        if (std::get<2>(item))
            obj.pushKV("Assigned", DateTimeStrFormat("%Y-%m-%d %H:%M:%S", std::get<3>(item)));
        else
            obj.pushKV("Removed", DateTimeStrFormat("%Y-%m-%d %H:%M:%S", std::get<3>(item)));
...
UniValue viewmyrestrictedaddresses(const JSONRPCRequest& request) {
    ...
    for (auto item : myRestrictedAddresses) {
        ...
        if (std::get<2>(item))
            obj.pushKV("Restricted", DateTimeStrFormat("%Y-%m-%d %H:%M:%S", std::get<3>(item)));
        else
            obj.pushKV("Derestricted", DateTimeStrFormat("%Y-%m-%d %H:%M:%S", std::get<3>(item)));
