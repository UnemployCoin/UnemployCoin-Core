...
{
    // Call into TxToUniv() in unemploycoin-common to decode the transaction hex.
    //
    // Blockchain contextual information (confirmations and blocktime) is not
    // available to code in unemploycoin-common, so we query them here and push the
            ...
            for (unsigned int i = 0; i < tx.vin.size(); i++) {
                ...
                if (GetSpentIndex(spentKey, spentInfo)) {
                    ...
                    if (spentInfo.addressType == 1) {
                        in.pushKV("address", CUnemployCoinAddress(CKeyID(spentInfo.addressHash)).ToString());
                    } else if (spentInfo.addressType == 2) {
                        in.pushKV("address", CUnemployCoinAddress(CScriptID(spentInfo.addressHash)).ToString());
                    }
                }
                newVin.push_back(in);
            }
...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "Outputs are addresses (paired with a UNEMP amount, data or object specifying an asset operation) or data.\n"
            "Returns hex-encoded raw transaction.\n"
            "Note that the transaction's inputs are not signed, and\n"
            "it is not stored in the wallet or transmitted to the network.\n"

            "\nPaying for Asset Operations:\n"
            "  Some operations require an amount of UNEMP to be sent to a burn address:\n"
            "\n"
            ...
            "     {\n"
            "       \"address\":                          (string, required) The destination unemploycoin address.\n"
            "                                               Each output must have a different address.\n"
            "         x.xxx                             (number or string, required) The UNEMP amount\n"
            "           or\n"
            ...
            // TODO if we decide to remove the consensus check from issue 675 https://github.com/UnemployCoinProject/UnemployCoin/issues/675
...
// TODO, if we decide to remove the consensus check https://github.com/UnemployCoinProject/UnemployCoin/issues/675, remove or add the code (requires consensus change)
...
//                    }
                    ...
                    // Create a new asset
                    CNewAsset asset(asset_name.get_str(), nAmount, units.getInt<int>(), reissuable.getInt<int>(), has_ipfs.getInt<int>(), DecodeAssetData(ipfs_hash.get_str()));
                ...
                else if (assetKey_ == "reissue")
                {
                    ...
                    if (!reissuable.isNull()) {
                        ...
                        int nReissuable = reissuable.getInt<int>();
                    ...
                    if (fHasOwnerChange && !IsValidDestinationString(owner_change_address.get_str()))
                        throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, owner_change_address is not a valid UnemployCoin address");
                    ...
                    // Create a new asset
                    CNewAsset asset(strAssetName, nAmount, units.getInt<int>(), reissuable.getInt<int>(), has_ipfs.getInt<int>(), DecodeAssetData(ipfs_hash.get_str()));
                    ...
                    if (fHasOwnerChange && !IsValidDestinationString(owner_change_address.get_str()))
                        throw JSONRPCError(RPC_INVALID_PARAMETER,
                                           "Invalid parameter, owner_change_address is not a valid UnemployCoin address");
                    ...
                    if (fHasRootChange && !IsValidDestinationString(root_change_address.get_str()))
                        throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, root_change_address is not a valid UnemployCoin address");
                    ...
                    for (int i = 0; i < (int)addresses.size(); i++) {
                        if (!IsValidDestinationString(addresses[i].get_str()))
                            throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, supplied address is not a valid UnemployCoin address");
                    }
...
UniValue decoderawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "         \"addresses\" : [           (json array of string)\n"
            "           \"12tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) unemploycoin address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
UniValue decodescript(const JSONRPCRequest& request)
{
    ...
    if (type.isStr() && type.get_str() == ASSET_TRANSFER_STRING) {
        ...
        r.pushKV("asset_name", transfer.strName);
        r.pushKV("amount", ValueFromAmount(transfer.nAmount));
        if (!transfer.message.empty())
            r.pushKV("message", EncodeAssetData(transfer.message));
        if (transfer.nExpireTime)
            r.pushKV("expire_time", transfer.nExpireTime);
        ...
        r.pushKV("asset_name", reissue.strName);
        ...
        if (reissue.strIPFSHash != "")
            r.pushKV("new_ipfs_hash", EncodeAssetData(reissue.strIPFSHash));
        ...
        if(AssetFromScript(script, asset, address)) {
            r.pushKV("asset_name", asset.strName);
            r.pushKV("amount", ValueFromAmount(asset.nAmount));
            r.pushKV("units", asset.units);
            ...
            if (hasIPFS)
                r.pushKV("ipfs_hash", EncodeAssetData(asset.strIPFSHash));
        }
        else if (OwnerAssetFromScript(script, ownerAsset, address))
        {
            r.pushKV("asset_name", ownerAsset);
            r.pushKV("amount", ValueFromAmount(OWNER_ASSET_AMOUNT));
