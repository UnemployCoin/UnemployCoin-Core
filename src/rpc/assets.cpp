...
UniValue UpdateAddressTag(const JSONRPCRequest &request, const int8_t &flag)
{
    ...
    if (!IsValidDestination(destination)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid UnemployCoin address: ") + address);
    }
    ...
    if (request.params.size() > 2) {
        ...
        if (!change_address.empty()) {
           ...
           if (!IsValidDestination(change_dest)) {
               throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid UnemployCoin change address: ") + change_address);
           }
        }
    }
    ...
    vTransfers.emplace_back(CAssetTransfer(tag_name, 1 * COIN, asset_data), change_address);
...
UniValue UpdateAddressRestriction(const JSONRPCRequest &request, const int8_t &flag)
{
    ...
    vTransfers.emplace_back(CAssetTransfer(restricted_name.substr(1, restricted_name.size()) + OWNER_TAG, 1 * COIN, asset_data), change_address);
...
UniValue issue(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 1 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            "4. \"change_address\"        (string), optional, default=\"\"), address the the unemp change will be sent to, if it is empty, change address will be generated for you\n"
        ...
        if (!change_address.empty()) {
            ...
            if (!IsValidDestination(destination)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                                   std::string("Invalid Change Address: Invalid UnemployCoin address: ") + change_address);
            }
        }
    }
...
UniValue issueunique(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 2 || request.params.size() > 5)
        throw std::runtime_error(
                ...
                "Five (5) UNEMP will be burned for each asset created.\n"
                ...
                "5. \"change_address\"        (string, optional, default=\"\"), address the the unemp change will be sent to, if it is empty, change address will be generated for you\n"
    ...
    if (!changeAddress.empty()) {
        ...
        if (!IsValidDestination(destination)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                               std::string("Invalid Change Address: Invalid UnemployCoin address: ") + changeAddress);
        }
    }
...
UniValue listassetbalancesbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"                  (string, required) a unemploycoin address\n"
...
UniValue getassetdata(const JSONRPCRequest& request)
{
    ...
    if (currentActiveAssetCache) {
        ...
        result.pushKV("amount", UnitValueFromAmount(asset.nAmount, asset.strName));
        ...
        if (asset.nHasIPFS) {
            if (asset.strIPFSHash.size() == 32) {
                result.pushKV("txid", EncodeAssetData(asset.strIPFSHash));
            } else {
...
UniValue listmyassets(const JSONRPCRequest &request)
{
    ...
    if (verbose) {
        for (; bal != end && bal != balances.end(); bal++) {
            ...
            asset.pushKV("balance", UnitValueFromAmount(bal->second, bal->first));
            ...
            asset.pushKV("outpoints", outpoints);
            result.pushKV(bal->first, asset);
        }
    }
    else {
        for (; bal != end && bal != balances.end(); bal++) {
...
UniValue listaddressesbyasset(const JSONRPCRequest &request)
{
    ...
    for (auto& pair : vecAddressAmounts) {
        result.pushKV(pair.first, UnitValueFromAmount(pair.second, asset_name));
    }
...
UniValue transfer(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 3 || request.params.size() > 7)
        throw std::runtime_error(
                ...
                "6. \"change_address\"       (string, optional, default = \"\") the transactions UNEMP change will be sent to this address\n"
    ...
    if (!IsValidDestination(to_dest)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid UnemployCoin address: ") + to_address);
    }
    ...
    std::string unemp_change_address = "";
    if (request.params.size() > 5) {
        unemp_change_address = request.params[5].get_str();
    }
    ...
    CTxDestination unemp_change_dest = DecodeDestination(unemp_change_address);
    if (!unemp_change_address.empty() && !IsValidDestination(unemp_change_dest))
        throw JSONRPCError(RPC_INVALID_PARAMETER, std::string("UNEMP change address must be a valid address. Invalid address: ") + unemp_change_address);
    ...
    CCoinControl ctrl;
    ctrl.destChange = unemp_change_dest;
...
UniValue transferfromaddresses(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 4 || request.params.size() > 8)
        throw std::runtime_error(
            "transferfromaddresses \"asset_name\" [\"from_addresses\"] qty \"to_address\" \"message\" expire_time \"unemp_change_address\" \"asset_change_address\"\n"
            + AssetActivationWarning() +
            ...
            "7. \"unemp_change_address\"       (string, optional, default = \"\") the transactions UNEMP change will be sent to this address\n"
    ...
    std::string unemp_change_address = "";
    if (request.params.size() > 6) {
        unemp_change_address = request.params[6].get_str();
    }
...
UniValue transferfromaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreAssetsDeployed() || request.params.size() < 4 || request.params.size() > 8)
        throw std::runtime_error(
                "transferfromaddress \"asset_name\" \"from_address\" qty \"to_address\" \"message\" expire_time \"unemp_change_address\" \"asset_change_address\"\n"
                + AssetActivationWarning() +
                ...
                "5. \"message\"                  (string, optional) Once RIP5 is voted in ipfs hash or txid hash to send along with the transfer\n"
                "6. \"expire_time\"              (numeric, optional) UTC timestamp of when the message expires\n"
                "7. \"unemp_change_address\"       (string, optional, default = \"\") the transaction UNEMP change will be sent to this address\n"
...
UniValue listassets(const JSONRPCRequest& request)
{
    ...
    for (auto data : assets) {
        ...
        if (verbose) {
            ...
            detail.pushKV("amount", UnitValueFromAmount(asset.nAmount, asset.strName));
...
UniValue getcacheinfo(const JSONRPCRequest& request)
{
    ...
    info.pushKV("asset total (exclude dirty)", (int)currentActiveAssetCache->DynamicMemoryUsage());

    UniValue descendants(UniValue::VOBJ);

    descendants.pushKV("asset address balance",   (int)memusage::DynamicUsage(currentActiveAssetCache->mapAssetsAddressAmount));
    descendants.pushKV("reissue data",   (int)memusage::DynamicUsage(currentActiveAssetCache->mapReissuedAssetData));

    info.pushKV("reissue tracking (memory only)", (int)memusage::DynamicUsage(mapReissuedAssets) + (int)memusage::DynamicUsage(mapReissuedTx));
    info.pushKV("asset data", descendants);
    info.pushKV("asset metadata map",  (int)memusage::DynamicUsage(passetsCache->GetItemsMap()));
    info.pushKV("asset metadata list (est)",  (int)passetsCache->GetItemsList().size() * (32 + 80)); // Max 32 bytes for asset name, 80 bytes max for asset data
    info.pushKV("dirty cache (est)",  (int)currentActiveAssetCache->GetCacheSize());
    info.pushKV("dirty cache V2 (est)",  (int)currentActiveAssetCache->GetCacheSizeV2());
...
UniValue listtagsforaddress(const JSONRPCRequest &request)
{
    ...
    CTxDestination dest = DecodeDestination(address);
    if (!IsValidDestination(dest))
        throw JSONRPCError(RPC_INVALID_PARAMETER, std::string("Not valid UNEMP address: ") + address);
...
UniValue checkaddresstag(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreRestrictedAssetsDeployed() || request.params.size() != 2)
        throw std::runtime_error(
                ...
                "\nArguments:\n"
                "1. \"address\"          (string, required) the UNEMP address to search\n"
...
UniValue issuerestrictedasset(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreRestrictedAssetsDeployed() || request.params.size() < 4 || request.params.size() > 9)
        throw std::runtime_error(
                ...
                "5. \"change_address\"        (string, optional, default=\"\") address that the unemp change will be sent to, if it is empty, change address will be generated for you\n"
...
UniValue reissuerestrictedasset(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreRestrictedAssetsDeployed() || request.params.size() < 3 || request.params.size() > 9)
        throw std::runtime_error(
                ...
                "6. \"change_address\"        (string, optional, default=\"\") address that the unemp change will be sent to, if it is empty, change address will be generated for you\n"
...
UniValue transferqualifier(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() > 3) {
        ...
        CTxDestination change_dest = DecodeDestination(change_address);
        if (!IsValidDestination(change_dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid UnemployCoin address: ") + change_address);
        }
    }
...
UniValue getsnapshot(const JSONRPCRequest& request)
{
    ...
    if (pAssetSnapshotDb->RetrieveOwnershipSnapshot(asset_name, block_height, snapshotDbEntry)) {
        result.pushKV("name", snapshotDbEntry.assetName);
        result.pushKV("height", snapshotDbEntry.height);

        UniValue entries(UniValue::VARR);
        for (auto const & ownerAndAmt : snapshotDbEntry.ownersAndAmounts) {
            UniValue entry(UniValue::VOBJ);

            entry.pushKV("address", ownerAndAmt.first);
            entry.pushKV("amount_owned", UnitValueFromAmount(ownerAndAmt.second, snapshotDbEntry.assetName));

            entries.push_back(entry);
        }

        result.pushKV("owners", entries);
...
UniValue purgesnapshot(const JSONRPCRequest& request)
{
    ...
    int block_height = 0;
    if (request.params.size() > 1) {
        block_height = request.params[2].getInt<int>();
    }
...
#ifdef ENABLE_WALLET
    { "assets",   "transferfromaddress",        &transferfromaddress,        {"asset_name", "from_address", "qty", "to_address", "message", "expire_time", "unemp_change_address", "asset_change_address"}},
    { "assets",   "transferfromaddresses",      &transferfromaddresses,      {"asset_name", "from_addresses", "qty", "to_address", "message", "expire_time", "unemp_change_address", "asset_change_address"}},
