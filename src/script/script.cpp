...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** UNEMP START */
    case OP_UNEMP_ASSET              : return "OP_UNEMP_ASSET";
    /** UNEMP END */
...
{
    if (this->size() > 31) {
        if ((*this)[25] == OP_UNEMP_ASSET) { // OP_UNEMP_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == UNEMP_R) { // Check to see if UNEMP starts at 27 ( this->size() < 105)
                if ((*this)[28] == UNEMP_V)
                    if ((*this)[29] == UNEMP_N)
                        index = 30;
            } else {
                if ((*this)[28] == UNEMP_R) // Check to see if UNEMP starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == UNEMP_V)
                        if ((*this)[30] == UNEMP_N)
                            index = 31;
            }
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == UNEMP_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == UNEMP_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == UNEMP_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == UNEMP_R) {
...
{
    return (this->size() > 23 &&
            (*this)[0] == OP_UNEMP_ASSET &&
            (*this)[1] == 0x14);
}

bool CScript::IsNullGlobalRestrictionAssetTxDataScript() const
{
    // 1 OP_UNEMP_ASSET followed by two OP_RESERVED + atleast 4 characters for the restricted name $ABC
    return (this->size() > 6 &&
            (*this)[0] == OP_UNEMP_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] == OP_RESERVED);
...
{
    // 1 OP_UNEMP_ASSET followed by one OP_RESERVED
    return (this->size() > 3 &&
            (*this)[0] == OP_UNEMP_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] != OP_RESERVED);
...
{
    CAmount nAmount;
    return (size() > 0 && *begin() == OP_RETURN) || (size() > 0 && *begin() == OP_UNEMP_ASSET) || (size() > MAX_SCRIPT_SIZE) || (GetAssetAmountFromScript(*this, nAmount) && nAmount == 0);
}
