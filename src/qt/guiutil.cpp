...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a UnemployCoin address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no unemploycoin: URI
    if(!uri.isValid() || uri.scheme() != QString("unemploycoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!RavenUnits::parse(RavenUnits::UNEMP, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
{
    // Convert unemploycoin:// to unemploycoin:
    //
    //    Cannot handle this later, because unemploycoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("unemploycoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "unemploycoin:");
    }
...
{
    QString ret = QString("unemploycoin:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(RavenUnits::format(RavenUnits::UNEMP, info.amount, false, RavenUnits::separatorNever));
        paramCount++;
    }
...
bool openRavenConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(UNEMPLOYCOIN_CONF_FILENAME);
    ...
    /* Open unemploycoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "UnemployCoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "UnemployCoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("UnemployCoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for UnemployCoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "unemploycoin.desktop";
    return GetAutostartDir() / strprintf("unemploycoin-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a unemploycoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=UnemployCoin\n";
        else
            optionFile << strprintf("Name=UnemployCoin (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the unemploycoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add unemploycoin app to startup item list
