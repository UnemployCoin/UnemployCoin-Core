...
public:
    explicit TxViewDelegate(const PlatformStyle *_platformStyle, QObject *parent=nullptr):
        QAbstractItemDelegate(parent), unit(RavenUnits::UNEMP),
    ...
    assetdelegate(new AssetViewDelegate(platformStyle, this))
{
    ...
    ui->unempBalancesLabel->setStyleSheet(STRING_LABEL_COLOR);
    ...
    ui->unempBalancesLabel->setFont(GUIUtil::getTopLabelFont());
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    // update the display unit, to not use the default ("UNEMP")
    updateDisplayUnit();
}
...
void OverviewPage::showAssets()
{
    if (AreAssetsDeployed()) {
        ...
        // This keeps the UNEMP balance grid from expanding and looking terrible when asset balance is hidden
