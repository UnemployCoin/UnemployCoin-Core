...
const int UNEMPLOYCOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString UNEMPLOYCOIN_IPC_PREFIX("unemploycoin:");
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/unemploycoin-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/unemploycoin-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/unemploycoin-paymentrequest";
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the unemploycoin: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(UNEMPLOYCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // unemploycoin: URI
        {
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    for (const QString& r : savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(UNEMPLOYCOIN_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = nullptr;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(UNEMPLOYCOIN_IPC_CONNECT_TIMEOUT);
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click unemploycoin: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start unemploycoin: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling unemploycoin: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in unemploycoin: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(UNEMPLOYCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // unemploycoin: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseRavenURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid UnemployCoin address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom unemploycoin addresses are not supported
        ...
        // UnemployCoin amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
