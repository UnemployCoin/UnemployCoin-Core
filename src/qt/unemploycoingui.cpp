...
std::array<CurrencyUnitDetails, 5> CurrencyUnits::CurrencyOptions = { {
    { "BTC",    "UNEMPBTC"  , 1,          8},
    { "mBTC",   "UNEMPBTC"  , 1000,       5},
    { "µBTC",   "UNEMPBTC"  , 1000000,    2},
    { "Satoshi","UNEMPBTC"  , 100000000,  0},
    { "USDT",   "UNEMPUSDT" , 1,          5}
} };
...
void RavenGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a UnemployCoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and unemploycoin: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to UNEMP addresses"));
...
//    messagingAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your UnemployCoin addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified UnemployCoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a unemploycoin: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible UnemployCoin command-line options").arg(tr(PACKAGE_NAME)));
...
void RavenGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        if(IconsOnly) {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/unemptext")));
        }
        else {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/unemploycointext")));
        }
...
//        m_toolbar->addAction(votingAction);
        ...
        labelCurrentMarket->setText(tr("UnemployCoin Market Price"));
        ...
        labelVersionUpdate->setText("<a href=\"https://github.com/RavenProject/UnemployCoin/releases\">New Wallet Version Available</a>");
        ...
        // Get the latest UnemployCoin release and let the user know if they are using the latest version
        // Network request code for the header widget
                       ...
                       if (key == "tag_name") {
                           ...
                           if (fNewSoftwareFound) {
                               ...
                               if (GetRandInt(2) == 1) {
                                   ...
                                   if (fRet) {
                                       QString link = "https://github.com/RavenProject/UnemployCoin/releases";
...
void RavenGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to UnemployCoin network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
{
    QString strTitle = tr("UnemployCoin"); // default title
...
void RavenGUI::checkAssets()
{
    ...
    if(AreAssetsDeployed()) {
        ...
        transferAssetAction->setToolTip(tr("Transfer assets to UNEMP addresses"));
...
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/RavenProject/UnemployCoin/releases"));
