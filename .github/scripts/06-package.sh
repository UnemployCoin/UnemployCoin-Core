...
cd ${GITHUB_WORKSPACE}
PKGVERSION=`grep "PACKAGE_VERSION" src/config/unemploycoin-config.h | cut -d\" -f2`
VERSION="${PKGVERSION}"
...
if [[ ${GITHUB_BASE_REF} =~ "release" ]]; then
    DISTNAME="unemploycoin-${VERSION}"
else
    DISTNAME="unemploycoin-${VERSION}-${SHORTHASH}"
fi
...
if [[ ${OS} == "windows" ]]; then
    ...
    for rmfile in detach-sig-create.sh win-codesign.cert unemploycoin-cli.exe unemploycoin-qt.exe unemploycoind.exe; do
        if [[ -e ${rmfile} ]]; then
            rm -f ${rmfile}
        fi
    done
    ...
    if [[ -e ${GITHUB_WORKSPACE}/dist/UnemployCoin-Qt.app/Contents/MacOS/install_cli.sh ]]; then
        chmod +x ${GITHUB_WORKSPACE}/dist/UnemployCoin-Qt.app/Contents/MacOS/install_cli.sh
    fi
    ...
    ${GITHUB_WORKSPACE}/depends/x86_64-apple-darwin14/native/bin/dmg dmg "UnemployCoin-Core.dmg" ${RELEASE_LOCATION}/${DISTNAME}-osx-unsigned.dmg
    ...
    if [[ -e ${RELEASE_LOCATION} ]]; then
        ...
        cd ${STAGE_DIR}
        cp -Rf ${DISTNAME}/bin/unemploycoind .
        cp -Rf ${DISTNAME}/bin/unemploycoin-cli .
    else
        echo "release directory doesn't exist"
    fi
